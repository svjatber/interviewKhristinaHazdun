...
import '../../App.css'
const TimerDesktop = ({timerDesktop,user}) =>{
    const [localCount, setLocalCount] = useState(timerDesktop)

    const setTime =  () =>{ // зміна значення таймеру
        setTimeout(  ()=>{
    ...
    const CountToDatabase = () =>{
        fireB.database().ref(`count/${user.uid}`).set({ count: localCount,changeMobile: true}); // зміна таймеру в database, changeMobile стає true при роботі таймеру десктопного, це дозволяє в іншій вкладці запускати таймер mobile
    }


    useEffect(()=>{ // передається з батьківської компоненти значення таймеру
